...
1. **Esc cannot leave the pause menu; only clicking Resume can.** This is a rule
   ...
   has exited the lock` and Next paints over the running game as a crash. A click
   on Resume is a fresh gesture, far enough after the release to be granted. The
...
2. **Re-locking retries, and never throws.** `requestLock` keeps asking every
   ...
   and **both must be swallowed**: an escaped one is caught by Next's dev overlay
...
3. **One rule owns the lock, driven by state rather than by buttons.** `Game.tsx`
...
4. **An unfocused tab holds no keys.** Alt-tabbing away delivers the keydown but
...
5. **Only the seeker takes the pointer lock.** A hider keeps their cursor — so
   ...
   `Game.tsx` while the seeker's arrives as a pointer-lock change. Neither can
   press Esc to leave the menu — see invariant 1.
6. **Jump is a velocity, not an impulse.** The seeker's collider is bigger and so
...
7. **Jump needs a real ground test *and* an edge trigger.** Grounded is a short
...
8. **Resizing the collider must move the body too.** A pose with a smaller
...
9. **A paused frame must still read a *complete* key state.** `Player` swaps in
...
10. **Pause freezes the mouse too, not just the keys.** Every pointer handler
...
11. **`RigidBody position` must be a stable array.** @react-three/rapier
...
12. **Seekers broadcast camera yaw, not body yaw**, so hiders can read where a
...
13. **The camera never leaves the arena.** `camera.ts` raycasts toward the desired
...
14. **A stroke in flight outranks every other meaning of the mouse.**
...
15. **Remote transforms are damped, never snapped** — except on the very first
    frame, or a joining player flies in from the origin.
16. **The shotgun is rate-limited here as well as on the server.** `lastShot`
...
17. **A hider climbs, and never reorients while doing it.** There is no grab
...
18. **On a wall, movement uses the wall's own axes, not the camera's.** `W` is up
...
19. **The probe reach depends on direction.** A hider is 0.26 wide and 1 tall, so
...
20. **Gravity is switched off per body, not globally.** `setGravityScale(0)` while
...
21. **Climbing off the top of something is not a special case.** The per-frame
...
22. **`RECLING_GRACE` and a push on release.** Without both, `Space` lets go and
    the next frame walks you straight back into the wall you are still touching,
    so it appears to do nothing.
23. **One wrap rule covers every edge.** `wrapCling` probes whatever you are
...
24. **Your own footsteps live in this file**, because it is the only place that
