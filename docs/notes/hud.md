...
- `DebugPanel.tsx` — the developer-mode readout, bottom left: **drawn fps and
  ...
  developer build, so it is absent from the built image — see
...
1. **This folder never imports from `world/`, `figure/`, `players/` or
   ...
   `Game.tsx` props and through `net/`. The one exception is reading `POSES` —
   for the legend's labels, and for the debug readout's numbers. Data, not
   behaviour. `LoadingScreen` is the shape this forces
   and is worth copying: the flag it reacts to is raised inside the Canvas by
   `world/Room.tsx`, so `Game.tsx` subscribes to `src/client/app/loading.ts` and the
   component itself takes no props at all and knows nothing about maps.
   `DebugPanel` is the same shape and is what shows it generalises: everything
   it knows about the *player* is written into `src/client/app/dev.ts` from inside the
   frame loop and sampled here, so a panel full of live physics state still
   imports nothing from `players/`. **Anything else the readout wants goes
   through that snapshot**, never through a new import.
...
23. **The debug readout samples; it is never driven by the frame loop.** It
    polls `src/client/app/dev.ts` on a 100 ms interval, so React re-renders it ten
    times a second against a game running at sixty. Both halves of that matter:
    a panel re-rendered per frame is a HUD that costs frames to look at, and
    digits changing sixty times a second are digits nobody can read. It is also
    `pointer-events-none`, like every other overlay that is not a control.

    **`DEV && ...` at the mount site is what keeps it out of the image**, and it
    has to stay a static condition for vite to fold it away — the panel and
    everything it imports are gone from the bundle, which was checked by
    grepping `dist/`. The one thing that does *not* disappear is its Tailwind
    classes: the CSS is generated by scanning source files, not by reading the
    bundle, so the built stylesheet carries about 1.5 kB it never uses. Known,
    and cheaper than the alternatives.
24. **The chip stays when the readout goes.** It is the toggle, so hiding it
    with everything else it switches would leave no way back — the panel above
    it collapses and the chip does not move, because it is anchored at the
    bottom of the column rather than at the top. It is also the **only**
    `pointer-events-auto` thing in this overlay: the readout must never eat a
    click meant for the world behind it. The keyboard half of the toggle lives
    in `Game.tsx`, with every other key this game listens for.
